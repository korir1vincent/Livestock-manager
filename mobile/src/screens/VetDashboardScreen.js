...
const VetDashboardScreen = ({ navigation }) => {
  ...
  const toggleAvailability = async () => {
    try {
      const api = getAuthenticatedAxios();
      const newStatus =
        vetProfile.availability === "Available" ? "Busy" : "Available";
  ...
  const filtered = consultations.filter((c) => {
    ...
    if (activeTab === "active")
      return ["Accepted", "In Progress"].includes(c.status);
    if (activeTab === "done")
      return ["Completed", "Cancelled", "Rejected"].includes(c.status);
    return true;
  });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          ...
          {vetProfile && (
            <Text style={styles.headerSub}>{vetProfile.specialty}</Text>
          )}
        </View>
        {vetProfile && (
          <TouchableOpacity
            style={[
              ...
              {
                backgroundColor:
                  vetProfile.availability === "Available"
                    ? "#10b981"
                    : "#6b7280",
              },
            ]}
                  ...
                  {consultation.status === "Pending" && (
                    <>
                      <Button
                        mode="contained"
                        onPress={() =>
                          handleRespond(consultation._id, "Accepted")
                        }
                        buttonColor="#10b981"
                      ...
                      <Button
                        mode="contained"
                        onPress={() =>
                          handleRespond(consultation._id, "Rejected")
                        }
                        buttonColor="#ef4444"
                        style={styles.actionBtn}
                        compact
